...
#calling the function to load data into dataframes
...
if add_sidebar == 'Aggregate Metrics':
    ...
    #Get date six month and twelve month before the max video publish time in the dataset
    metric_date_6mo = df_agg_metrics['Video publish time'].max() - pd.DateOffset(months=6)
    metric_date_12mo = df_agg_metrics['Video publish time'].max() - pd.DateOffset(months=12)

    #Get past six months data and take the median of all the numeric columns
    df_agg_metrics_6mo = df_agg_metrics[df_agg_metrics['Video publish time'] >= metric_date_6mo]
    df_agg_metrics_6mo_num_cols = df_agg_metrics_6mo.iloc[:,1:]
    metric_medians6mo = df_agg_metrics_6mo_num_cols.median()

    #Get past one year data and take the median of all the numeric columns
    df_agg_metrics_12mo = df_agg_metrics[df_agg_metrics['Video publish time'] >= metric_date_12mo]
    df_agg_metrics_12mo_num_cols = df_agg_metrics_12mo.iloc[:,1:]
    metric_medians12mo = df_agg_metrics_12mo_num_cols.median()

    #Display metrics data in five columns on streamlit app
